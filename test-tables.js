testTables = {
    vectorconfig: {
        data: new Matrix(
            new NumericVector(27,27,27,19,30,31,30,30,30,30,30,23,19,27,28,25,26,30,11,25,28,25,28,28,36,32,34,29,25,27).name("values").label("naměřené hodnoty").formatter(function(v,i,a) {return v < a.avg() ? "podprůměr" : "nadprůměr"}),
            new StringVector("A","A","A","A","A","A","B","B","B","B","B","B","C","C","C","C","C","C","D","D","D","D","D","D","E","E","E","E","E","E").name("group").label("skupina respondentů").formatter({"A": "adolescenti", "B": "blbci", "C": "cisterciáci", "D": "debilová", "E": "emilové"})
            ),
    ...
    muvsanova1: {
        data: new Matrix(
            new NumericVector(27,27,27,19,30,31).name("A").label("Aš"),
            new NumericVector(30,30,30,30,30,23).name("B").label("Brno"),
            new NumericVector(19,27,28,25,26,30).name("C").label("Chlumec nad Cidlinou"),
            new NumericVector(11,25,28,25,28,28).name("D").label("Dobřicovice"),
            new NumericVector(36,32,34,29,25,27).name("E").label("Ejpovice")
        ),
    ...
    muvsanova2: {
        data: new Matrix(
            new NumericVector(27,27,27,19,30,31,30,30,30,30,30,23,19,27,28,25,26,30,11,25,28,25,28,28,36,32,34,29,25,27).name("values").label("naměřené hodnoty"),
            new StringVector("A","A","A","A","A","A","B","B","B","B","B","B","C","C","C","C","C","C","D","D","D","D","D","D","E","E","E","E","E","E").name("group").label("skupina").formatter({A: "Aš", B: "Brno", C: "Chlumec na Cidlinou", D: "Dobřichovice", E: "Ejpovice"})
        ),
        args: {vectors: [0], factor: 1},
        method: "anovaow"
    },
    ...
    ancova: {
        ...
        args: {factor: 0, dependent: 2, covariant: 1},
        method: "ancova"
    },
    muvscontingency1: {
        data: new Matrix(
            new StringVector("ZŠ","ZŠ","ZŠ","ZŠ","SŠ","SŠ","SŠ","SŠ","VŠ","VŠ","VŠ","VŠ").name("education").label("dosažené vzdělání").formatter({"ZŠ": "základní škola", "SŠ": "střední škola", "VŠ": "vysoká škola"}),
    ...
    kwanova1: {
        data: new Matrix(
            new NumericVector(7,14,14,13,12,9,6,14,12,8).name("F1").label("fertilizer 1"),
            new NumericVector(15,17,13,15,15,13,9,12,10,8).name("F2").label("fertilizer 2"),
            new NumericVector(6,8,8,9,5,14,13,8,10,9).name("F3").label("fertilizer 3")
        ),
        args: {vectors: [0,1,2]},
        method: "kwanova"
    },
